- **Der Skill nennt jetzt alle seine Befehle.** `serie`, `einlesen`, `preview`, `init` und `mcp`
  ...
  Dabei fiel eine Lücke in Regel 0 auf: **`preview` prüft die Eingabe nicht und misst nichts
  nach.** Es setzt auch, was `render` mit Code 1 ablehnt — nachgewiesen an einem Brief mit einem
  unbekannten Frontmatter-Feld. Das steht jetzt dabei, statt es jemanden herausfinden zu lassen.
